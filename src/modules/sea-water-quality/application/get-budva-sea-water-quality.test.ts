...
test("allows the sea water quality cache for every capability-supported city", () => {
  ...
  assert.equal(canReadBudvaSeaWaterQuality(bar), true);
...
function snapshotFor(
  municipality: "bar" | "budva" | "kotor" | "tivat",
...
test("Bar, Budva, Kotor and Tivat cache paths are independent files that never cross-contaminate each other's data", async () => {
  const directory = await mkdtemp(join(tmpdir(), "sea-water-quality-isolation-"));
  try {
    const barPath = join(directory, "bar-sea-water-quality.json");
    ...
    assert.notEqual(barPath, budvaPath);
    assert.notEqual(barPath, kotorPath);
    assert.notEqual(barPath, tivatPath);
    assert.notEqual(budvaPath, tivatPath);
    assert.notEqual(budvaPath, kotorPath);
    assert.notEqual(kotorPath, tivatPath);

    await writeBudvaSeaWaterQualityCache(snapshotFor("bar", 15), barPath);
    ...
    const barResult = await getCachedBudvaSeaWaterQuality(
      barPath,
    ...
    assert.equal(barResult.summary?.municipality, "bar");
    assert.equal(barResult.summary?.totalLocations, 15);
...
test("getSeaWaterQualityCachePath — the mechanism getBudvaSeaWaterQuality resolves per context.city.id — never collides across supported cities", () => {
  ...
  assert.notEqual(getSeaWaterQualityCachePath("bar"), getSeaWaterQualityCachePath("budva"));
  assert.notEqual(getSeaWaterQualityCachePath("bar"), getSeaWaterQualityCachePath("kotor"));
  assert.notEqual(getSeaWaterQualityCachePath("bar"), getSeaWaterQualityCachePath("tivat"));
