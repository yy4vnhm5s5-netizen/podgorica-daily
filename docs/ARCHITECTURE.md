...
The central city registry supplies a route-derived `CityContext` with city, locale, and timezone to providers and application use cases. Bar, Budva, Kotor, Podgorica, and Tivat are active; inactive entries remain planning data only and do not create routes or collection work. Bar currently declares only weather and electricity capabilities, so public routing and dashboard composition expose only those modules. Event records carry one stable `cityId` and retain `cityIds` for established cross-city contracts. City content uses active city-prefixed canonical routes. The platform root (`/`) is its own canonical city-selection homepage; it does not render a duplicate city dashboard. Product and UX decisions for that surface are defined in [PRODUCT_VISION.md](PRODUCT_VISION.md) and [UX_ARCHITECTURE.md](UX_ARCHITECTURE.md).
...
`src/modules/flights` owns the shared `Flight` domain, explicit Podgorica and Tivat Airports of Montenegro feed configuration, source-specific parsers, city-isolated atomic snapshots and locks, a cache-backed application query, and dashboard/full-schedule presentation. Podgorica retains `.runtime/cache/podgorica-flights.json`; Tivat writes `.runtime/cache/tivat-flights.json`. One collector refreshes every configured active airport while retaining failures per city; no visitor request calls an airport feed. See ADR 0024.

`src/modules/going-out` is intentionally separate from the Event Platform. It owns a city-aware nightlife read model, an explicitly allow-listed MonteGigs listing adapter, per-city atomic snapshots and locks, a collector, a cache-backed query, and city dashboard/listing presentation. The supported source mapping is explicit rather than slug-derived: Podgorica retains `.runtime/cache/montegigs-going-out.json`, while Budva uses a separate module-owned cache file. A request or parser failure preserves only that city's valid snapshot; visitor requests read cached data only. Eligible current records expose a provider-prefixed internal detail URL (`/<city>/izlasci/montegigs-<sourceEventId>`) resolved solely from that normal public snapshot; the private detail-enrichment cache is never publicly read. Missing source times remain unavailable rather than inferred or rendered as placeholders. See ADR 0020.
...
Event records have one required `cityId` and retain `cityIds` for existing cross-city contracts; trusted source references survive exact or strong deterministic merges. CNP, `glavni-grad-podgorica`, `tourism-podgorica`, and `cineplexx-podgorica` are registered cache-backed active providers. KIC collection code and fixtures are retained but excluded from public reads, startup initialization, and recurring refreshes while its upstream TLS certificate is expired. Cineplexx is rendered from its single public Podgorica programme page by a bounded server-side browser process, never by a visitor request. The public, mobile-first presentation routes are `src/app/[city]/dogadjaji/page.tsx` and `src/app/[city]/dogadjaji/[eventId]/page.tsx`; only active city slugs are publicly routable. Each live provider requires `ENABLE_EVENTS=true` and `EVENT_PROVIDER_MODE=live`; mock and disabled modes do not expose a live provider. Provider failures remain isolated.
