...
The central city registry supplies a route-derived `CityContext` with city, locale, and timezone to providers and application use cases. Podgorica and Budva are active; inactive entries remain planning data only and do not create routes or collection work. Budva currently declares only weather, Going Out, and electricity capabilities, so public routing and dashboard composition expose only those modules. Event records carry one stable `cityId` and retain `cityIds` for established cross-city contracts. City content uses active city-prefixed canonical routes. The platform root (`/`) is its own canonical city-selection homepage; it does not render a duplicate city dashboard. Product and UX decisions for that surface are defined in [PRODUCT_VISION.md](PRODUCT_VISION.md) and [UX_ARCHITECTURE.md](UX_ARCHITECTURE.md).
