...
`src/modules/going-out` is intentionally separate from the Event Platform. It owns a small Podgorica nightlife read model, MonteGigs listing adapter, atomic cache, collector, cache-backed query, and the dashboard and `/podgorica/izlasci` presentation. The collector reads only the public `https://staging.montegigs.me/me/events/podgorica` listing, keeps only future Podgorica records, and writes `.runtime/cache/montegigs-going-out.json`. It preserves a valid snapshot when a request or parser fails; visitor requests read that cache only. Missing source times remain unavailable rather than inferred or rendered as placeholders. See ADR 0020.
