...
Current weather for Podgorica is delivered through an isolated Open-Meteo adapter with attribution, freshness, and safe loading, empty, and error states. The homepage retains a city-configured BusTicket4.me external link, but does not ingest or show bus departures. The ŽPCG railway module collects official timetable departures into `.runtime/cache/zpcg-railway-departures.json` and displays cache-backed upcoming departures from Podgorica. The Flights module collects configured official Airports of Montenegro feeds into city-isolated snapshots, including `.runtime/cache/podgorica-flights.json` and `.runtime/cache/tivat-flights.json`, and exposes cache-backed schedules for supported airport cities. Broader transport coverage, real-time traffic, and route planning remain separate scopes.

The standalone `Izlasci` module collects public upcoming Podgorica music, nightlife and performance listings from MonteGigs into `.runtime/cache/montegigs-going-out.json`. It remains separate from the official-source Event Platform, uses cache-only public reads, and preserves unavailable source times rather than inferring them. Source policy must be rechecked with MonteGigs before production rollout. See ADR 0020.
