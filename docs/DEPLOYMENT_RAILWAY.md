...
| Parking, every 10 minutes                                      | `*/10 * * * *`            | `/api/internal/parking/refresh`           | `PARKING_REFRESH_SECRET`           |
...
# Parking: /api/internal/parking/refresh + PARKING_REFRESH_SECRET, every 10 minutes.
# Enable it only with ENABLE_PARKING=true on the Web service. The endpoint invokes the collector
# on the mounted Web-service volume; the curl cron never mounts or writes that volume.
sh -c 'curl --fail-with-body --silent --show-error --max-time 120 --write-out "\n" --request POST --header "Authorization: Bearer $REFRESH_SECRET" "$REFRESH_URL"'
