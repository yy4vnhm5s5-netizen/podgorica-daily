...
`PORT` is Railway-provided. `NODE_ENV`, `NEXT_PUBLIC_APP_ENV`, `DEFAULT_CITY`, `ENABLE_EVENTS`, and `EVENT_PROVIDER_MODE` are safe configuration. `NEXT_PUBLIC_SITE_URL` is public and required for absolute production metadata. `EVENT_REFRESH_SECRET` is a server-only secret and must never be committed or exposed to the browser. `EVENT_CACHE_DIR` and provider cache paths, including optional `CINEPLEXX_EVENT_CACHE_PATH`, must be mounted-volume paths in production. Keep `CINEPLEXX_CACHE_FRESHNESS_MINUTES=780` unless the twice-daily collection policy changes. Weather currently has no API key. `.env.example` contains safe defaults only.
