...
2. Create one service named `web`. Railway detects `railway.toml`; confirm `pnpm run build`, `pnpm run start -- --hostname 0.0.0.0 --port $PORT`, and health path `/api/health`.
