...
5. Generate Railway's public `*.up.railway.app` domain in Networking. Set `NEXT_PUBLIC_SITE_URL` to that exact HTTPS origin and redeploy. Verify `/api/health`, `/dogadjaji`, the permanent `/events`, `/me/dogadjaji`, and `/en/events` redirects, and an event detail URL.
