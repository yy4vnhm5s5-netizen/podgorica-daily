...
For upcoming listing records only, the collector may also request up to 12 distinct same-host event details per city refresh, with at most three logical requests in flight. Detail pages contribute only explicit description, address, organizer and source-labelled external information URL fields; JSON-LD is preferred when it matches the requested source event, while labelled page sections are a fail-open fallback. A failed, redirected, malformed or mismatched detail never removes its listing record or replaces the entire snapshot. Visitor requests never fetch MonteGigs. See ADR 0020.
