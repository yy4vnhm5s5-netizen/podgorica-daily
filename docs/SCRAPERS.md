...
## Airports of Montenegro flights

Flights are collected only from the public first-party feeds used by the official Airports of Montenegro status pages: [Podgorica](https://montenegroairports.com/aerodrom-podgorica/) uses `https://montenegroairports.com/aerodromixs/cache-flights.php?airport=pg`, while [Tivat](https://montenegroairports.com/aerodrom-tivat/) uses the same approved HTTPS endpoint with `airport=tv`. The airport configuration explicitly selects the schema: Podgorica returns a `{ value: [] }` payload with `FlightType` and ISO-like scheduled timestamps; Tivat returns an array with `TipLeta` (`I`/`O`), `Datum` (`dd-MM-yyyy`) and `Planirano` (`HHmm`). `pnpm run collect:airport-flights` validates only those configured selectors, applies the existing ten-second timeout, bounded retries, response-size checks, and source-specific Zod validation before normalization. Podgorica retains `.runtime/cache/podgorica-flights.json`; Tivat writes `.runtime/cache/tivat-flights.json`. Snapshots and locks are city-specific, so one airport failure retains only its own valid snapshot and does not block the other. Public dashboard and `/[city]/letovi` reads never request the airport source. The bundled scheduler runs the one collector every 15 minutes. `pnpm run collect:podgorica-flights` remains a compatibility alias. Tests use saved fixtures and injected HTTP only. See ADR 0024.

## Parking servis Podgorica availability

Parking is a Podgorica-only, cache-backed feature. `pnpm run collect:parking` uses the official [availability page](https://parkingservispg.me/slobodna-mjesta/) only for the verified static catalogue and its documented machine endpoint `https://parkingservispg.me/wp-content/plugins/parking-mjesta/parking.php` for free-space counts. The endpoint currently labels valid JSON as `text/html`; the client deliberately reads bounded response text and parses JSON explicitly rather than trusting the header. It uses an ordinary allow-listed HTTPS fetch with a ten-second timeout, no cookies, no browser emulation and no retries. The repeated static HTML fallback value is never ingested.

Only the fourteen `parking_id` values that the official page itself targets are published: ten parking lots (`broj1`, `broj2`, `broj2a`, `broj3`, `broj5`, `broj6`, `broj8`, `broj9`, `broj11`, `broj12`) and four garages (`garaza1`–`garaza4`). Their public names, types and capacities live in the collector-owned catalogue; API `name` is optional and does not override it. Each accepted record is independently validated against that catalogue, its capacity and a plausible Unix source timestamp. Unknown, duplicate, malformed or implausible records are not published and produce concise warnings. The Podgorica snapshot is atomically written to `.runtime/cache/parking-podgorica-availability.json`; a failed refresh retains the previous snapshot.

Freshness is per location: only a source timestamp no older than 15 minutes may display a free-space count. Stale or missing records stay visible as static official locations but state that availability is currently unavailable. Public `/podgorica/parking` requests only read the local snapshot. The bundled scheduler checks the source every ten minutes under the provider lock; tests use saved JSON fixtures and injected HTTP only.
