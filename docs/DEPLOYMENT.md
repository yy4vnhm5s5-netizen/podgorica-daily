...
- `RUNTIME_DATA_DIR` is the shared root for file-backed provider caches. It defaults to `.runtime` locally. Without an explicit per-provider override, CEDIS resolves to `<RUNTIME_DATA_DIR>/cache/cedis-planned-outages.json`, VIK to `<RUNTIME_DATA_DIR>/cache/vikpg-water-alerts.json`, AMSCG to `<RUNTIME_DATA_DIR>/cache/amscg-road-conditions.json`, and MonteGigs Going Out to `<RUNTIME_DATA_DIR>/cache/montegigs-going-out.json`. Set `ENABLE_GOING_OUT=false` to keep the MonteGigs module hidden and prevent boot initialization.
...
6. Install the Caddy template as the active site configuration, replace placeholders, validate it with `caddy validate`, then reload Caddy. Verify HTTPS, `/dogadjaji`, the permanent `/events`, `/me/dogadjaji`, and `/en/events` redirects, and an event detail URL on a narrow phone viewport.
