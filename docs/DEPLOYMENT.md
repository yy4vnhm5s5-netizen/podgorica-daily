...
- `RUNTIME_DATA_DIR` is the shared root for file-backed provider caches. It defaults to `.runtime` locally. CEDIS preserves `<RUNTIME_DATA_DIR>/cache/cedis-planned-outages.json` for Podgorica and derives isolated sibling snapshots for other approved municipalities; VIK resolves to `<RUNTIME_DATA_DIR>/cache/vikpg-water-alerts.json`, and MonteGigs Going Out to `<RUNTIME_DATA_DIR>/cache/montegigs-going-out.json`. Set `ENABLE_GOING_OUT=false` to keep the MonteGigs module hidden and prevent boot initialization.
