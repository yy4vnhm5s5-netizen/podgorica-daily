...
- The contact form delivers only through server-side SMTP, via the Zoho Mail EU relay. Set `CONTACT_EMAIL`, `SMTP_FROM`, `SMTP_HOST=smtppro.zoho.eu`, `SMTP_PORT=465`, and `SMTP_SECURE=true`; provide `SMTP_USERNAME` and `SMTP_PASSWORD` together (the relay requires authentication). Do not expose any of these values to the browser. Without a complete SMTP configuration, the form returns a safe delivery-unavailable state and does not claim that an inquiry was sent.
