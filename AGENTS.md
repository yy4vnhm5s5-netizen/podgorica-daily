...
Use Node's built-in test runner for focused module unit tests. The configured `pnpm run test` suite covers Daily Overview, Event Platform domain/query/provider boundaries, fixture-backed CEDIS, VIK, AMSCG, KIC, CNP, Glavni Grad, Tourism, Cineplexx, and ŽPCG parser, refresh, cache, collector, and application-integration paths, plus Events presentation filtering and grouping. Tests must not make live provider requests.
...
- Daily Overview consumes cached normalized data only. CEDIS, AMSCG, and VIK Podgorica are approved source adapters: collection is invoked by `pnpm run collect:cedis`, `pnpm run collect:amscg`, or `pnpm run collect:vikpg`, never by a page request. Preserve allowed-host validation, fixture tests, stale/unavailable behaviour, provider-mode boundaries, and `CityContext` input. Podgorica is the only active city; city routes are derived from the active registry and a future city requires approved source coverage before activation. The file cache is not durable on serverless deployments; add a durable adapter only after its contract and operations are approved.
