...
The accepted ADRs are `docs/adr/0001-modular-monolith.md` through `docs/adr/0017-cineplexx-podgorica-rendered-programme-provider.md`.
...
- Flights collection is limited to `pnpm run collect:podgorica-flights`, which fetches only current and following Podgorica-local dates from the official Airports of Montenegro schedule, writes `.runtime/cache/podgorica-flights.json` atomically, and never runs in a visitor request. Preserve its official-host validation, ten-second timeout, one retry, response-size limit, fixture tests, cache-retention behaviour, and Podgorica timezone handling. The homepage and `/me/letovi` must use the same cache-backed application query. See ADR 0018.
