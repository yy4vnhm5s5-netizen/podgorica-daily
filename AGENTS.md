...
The repository currently contains a production-oriented foundation, design system, application shell, a server-rendered Weather module for current Podgorica conditions, a deterministic Daily Overview, and City Alerts. City Alerts reads cached official CEDIS planned power outages when a collection snapshot is available; mock alerts remain an explicitly configured development mode. Daily Overview is generated by deterministic rules over cached structured data and consumes only generic alert read-model data. Transport, events, persistence, authentication, maps, and search features are not implemented. Do not describe shell placeholders as implemented product capabilities.
...
Use Node's built-in test runner for focused module unit tests. The configured `pnpm run test` suite covers Daily Overview and fixture-backed CEDIS parser, cache, refresh, provider-mode, and presentation-copy paths.
...
The accepted ADRs are `docs/adr/0001-modular-monolith.md` through `docs/adr/0007-cedis-cached-planned-outages.md`.
...
- Never implement transport, events, maps, search, identity, administration, scraping, or persistence without explicit approved scope. The approved CEDIS collector is the exception for its documented source and cache boundary. Extend Weather only through its module boundary and approved provider policy.
...
- Daily Overview consumes cached normalized data only. CEDIS is the approved first source adapter: collection is invoked by `pnpm run collect:cedis`, never by a page request. Preserve its allowed-host validation, fixture tests, stale/unavailable behaviour, and `CEDIS_PROVIDER_MODE` boundary. The file cache is not durable on serverless deployments; add a durable adapter only after its contract and operations are approved.
