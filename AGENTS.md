...
The root dashboard route is `src/app/page.tsx`; the root layout is `src/app/layout.tsx`. Production exposes Montenegrin on root-level canonical routes, including `/dogadjaji`, `/izlasci`, `/letovi`, `/kontakt`, `/struja`, and legal pages. English locale infrastructure and translations remain retained, while legacy `/me/*` and `/en/*` requests receive permanent redirects through `src/middleware.ts`. Legacy `/events` redirects to `/dogadjaji`. The dashboard combines Weather, Daily Overview, cache-backed City Alerts, cache-backed Going Out, cache-backed Events and Cineplexx programme data, cache-backed Podgorica Airport flights, and cache-backed ŽPCG departures. Search and command palette are not exposed.
...
The accepted ADRs are `docs/adr/0001-modular-monolith.md` through `docs/adr/0019-podgorica-airport-public-flight-feed.md`; ADR 0018 is superseded.
...
- Daily Overview consumes cached normalized data only. CEDIS, AMSCG, and VIK Podgorica are approved source adapters: collection is invoked by `pnpm run collect:cedis`, `pnpm run collect:amscg`, or `pnpm run collect:vikpg`, never by a page request. Preserve allowed-host validation, fixture tests, stale/unavailable behaviour, provider-mode boundaries, and `CityContext` input. Podgorica is the only enabled city; do not expose city routes or selectors until separately approved. The file cache is not durable on serverless deployments; add a durable adapter only after its contract and operations are approved.
...
- Events presentation is public and mobile-first at `/dogadjaji` with stable-ID detail pages at `/dogadjaji/<eventId>`. Keep listing/detail reads cache-only and accepted-only; URL filters apply after the application service returns its normalized read model. The native filter dialog is the limited client boundary. Preserve semantic landmarks, keyboard focus management, source attribution, cancellation/postponement text, and localized date formatting. Never fetch a provider, run a collector, or expose rejection diagnostics from these routes.
