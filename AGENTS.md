...
The root route in `src/app/page.tsx` redirects to the default locale. The locale dashboard route is `src/app/[locale]/page.tsx`, and the root layout is `src/app/layout.tsx`. The dashboard combines Weather, Daily Overview, cache-backed City Alerts, and static placeholders for unimplemented capabilities; search and command-palette messages are non-interactive placeholders.
...
Use Node's built-in test runner for focused module unit tests. The configured `pnpm run test` suite covers Daily Overview, Event Platform domain/query/provider boundaries, fixture-backed CEDIS paths, and fixture-backed KIC, CNP, Glavni Grad, and Tourism parser, refresh, cache, collector, and application-integration paths. Tests must not make live provider requests.
...
- Never implement transport, maps, search, identity, administration, scraping, or persistence without explicit approved scope. The approved CEDIS, AMSCG, KIC, CNP, Glavni Grad, and Tourism collectors are exceptions for their documented source and cache boundaries. Do not add another event source, collector, UI, route, or visible mock data without new scope. Extend Weather only through its module boundary and approved provider policy.
