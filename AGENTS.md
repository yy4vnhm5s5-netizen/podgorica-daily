...
The repository currently contains a production-oriented foundation, design system, application shell, a server-rendered Weather module for current Podgorica conditions, a deterministic Daily Overview, and City Alerts. City Alerts reads cached official CEDIS planned power outages, AMSCG road conditions, and VIK Podgorica water-service notices when collection snapshots are available; mock CEDIS alerts remain an explicitly configured development mode. Daily Overview is generated by deterministic rules over cached structured data and consumes only generic alert read-model data. The Events module has cache-backed KIC, CNP, Glavni Grad Podgorica, and Turistička organizacija Podgorice providers and collectors, plus a mobile-first public listing and detail UI that reads accepted cached records only. The homepage also has a city-configured external bus-station link card; it does not collect or display departure data. Persistence, authentication, maps, and search features are not implemented. Do not describe shell placeholders as implemented product capabilities.
...
Use Node's built-in test runner for focused module unit tests. The configured `pnpm run test` suite covers Daily Overview, Event Platform domain/query/provider boundaries, fixture-backed CEDIS paths, fixture-backed KIC, CNP, Glavni Grad, and Tourism parser, refresh, cache, collector, and application-integration paths, plus Events presentation filtering and grouping. Tests must not make live provider requests.
...
- Never implement transport, maps, search, identity, administration, scraping, or persistence without explicit approved scope. The city-configured BusTicket4.me station link is the only approved transport exception; it must remain link-only unless a separately approved source contract permits collection. The approved CEDIS, AMSCG, VIK Podgorica, KIC, CNP, Glavni Grad, and Tourism collectors are exceptions for their documented source and cache boundaries. Do not add another event source, collector, route, or visible mock data without new scope. Extend the existing Events UI only through its presentation/application boundaries and approved provider policy. Extend Weather only through its module boundary and approved provider policy.
