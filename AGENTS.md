...
The accepted ADRs are `docs/adr/0001-modular-monolith.md` through `docs/adr/0015-tourism-podgorica-event-provider.md`.
...
- Never implement transport, maps, search, identity, administration, scraping, or persistence without explicit approved scope. The approved CEDIS, AMSCG, KIC, CNP, Glavni Grad, and Tourism collectors are exceptions for their documented source and cache boundaries. Do not add another event source, collector, route, or visible mock data without new scope. Extend the existing Events UI only through its presentation/application boundaries and approved provider policy. Extend Weather only through its module boundary and approved provider policy.
