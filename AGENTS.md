...
The repository currently contains a production-oriented foundation, design system, application shell, a server-rendered Weather module for current Podgorica conditions, and explicitly labelled internal Daily Overview and City Alerts demos. Both demos use mocked content only; Daily Overview is generated by deterministic rules over cached structured data, and City Alerts do not represent live disruptions. Transport, events, persistence, authentication, scraping, maps, and search features are not implemented. Do not describe shell placeholders or either demo as implemented product capabilities.
...
The root route in `src/app/page.tsx` redirects to the default locale. The locale dashboard route is `src/app/[locale]/page.tsx`, and the root layout is `src/app/layout.tsx`. The dashboard combines the Weather module, the clearly labelled Daily Overview and City Alerts demos, and static placeholders for unimplemented capabilities; search and command-palette messages are non-interactive placeholders.
...
Use Node's built-in test runner for focused module unit tests. The configured `pnpm run test` suite currently covers the Daily Overview generator.
...
The accepted ADRs are `docs/adr/0001-modular-monolith.md`, `docs/adr/0002-shared-ui-primitives.md`, `docs/adr/0003-open-meteo-current-weather.md`, `docs/adr/0004-locale-prefixed-internationalization.md`, `docs/adr/0005-deterministic-daily-overview.md`, and `docs/adr/0006-city-alerts-demo-provider.md`.
...
- Never implement transport, events, maps, search, identity, administration, scraping, or persistence without explicit approved scope. Extend Weather only through its module boundary and approved provider policy.
...
- Daily Overview consumes cached normalized data only. Add scheduled providers, source adapters, and cache persistence only after contracts, provenance, freshness, monitoring, and failure behaviour are approved; page requests must never trigger collection.
