...
The platform root route is `src/app/page.tsx`; the root layout is `src/app/layout.tsx`. Production exposes the Gradom.me platform homepage at `/` and capability-specific Montenegrin city content on canonical city-prefixed routes. Podgorica exposes `/podgorica`, `/podgorica/dogadjaji`, `/podgorica/izlasci`, `/podgorica/letovi`, and `/podgorica/struja`; Budva exposes `/budva`, `/budva/izlasci`, and `/budva/struja`. Contact and legal pages remain global root routes. The static city registry resolves a `CityContext` at route boundaries; only active cities receive public routes. English locale infrastructure and translations remain retained but no locale or compatibility redirect layer is exposed. The dashboard combines only capabilities declared for each city. Search and command palette are not exposed.
...
The accepted ADRs are `docs/adr/0001-modular-monolith.md` through `docs/adr/0023-platform-root-homepage.md`; ADRs 0018 and 0021 are superseded, and ADR 0023 supersedes the root-route and sitemap portions of ADR 0022.
...
- Daily Overview consumes cached normalized data only. CEDIS and VIK Podgorica are approved source adapters: collection is invoked by `pnpm run collect:cedis` or `pnpm run collect:vikpg`, never by a page request. Preserve allowed-host validation, fixture tests, stale/unavailable behaviour, provider-mode boundaries, and `CityContext` input. City routes are derived from the active registry; Podgorica and Budva are active, while a future city requires approved source coverage before activation. The file cache is not durable on serverless deployments; add a durable adapter only after its contract and operations are approved.
