...
Use Node's built-in test runner for focused module unit tests. The configured `pnpm run test` suite covers Daily Overview, the Event Platform’s deterministic domain/query/provider boundaries, and fixture-backed CEDIS parser, cache, refresh, provider-mode, and presentation-copy paths.
...
The accepted ADRs are `docs/adr/0001-modular-monolith.md` through `docs/adr/0010-event-platform-foundation.md`.
...
- Never implement transport, maps, search, identity, administration, scraping, or persistence without explicit approved scope. The approved CEDIS and AMSCG collectors are exceptions for their documented source and cache boundaries. The Event Platform is approved only as a disabled internal foundation: do not add an event source, collector, UI, route, or visible mock data without new scope. Extend Weather only through its module boundary and approved provider policy.
