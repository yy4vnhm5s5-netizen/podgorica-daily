...
- Never implement transport, maps, search, identity, administration, scraping, or persistence without explicit approved scope. The approved CEDIS, AMSCG, KIC, and CNP collectors are exceptions for their documented source and cache boundaries. Do not add another event source, collector, UI, route, or visible mock data without new scope. Extend Weather only through its module boundary and approved provider policy.
