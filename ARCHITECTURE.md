...
|---|---|---|---|
| CEDIS (power outages, active mapped cities) | `city-alerts` | `pnpm run collect:cedis` / `/api/internal/cedis/refresh` | `cedis-planned-outages.json` (Podgorica), with isolated `cedis-planned-outages-<cityId>.json` siblings for Bar, Budva, Kotor, and Tivat — see note below |
